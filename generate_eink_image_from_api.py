...
def get_dynamic_font(font_path, text, max_w, max_h, start_size=24, min_size=12, is_title=False):
    ...
    for size in range(start_size, min_size - 1, -2):
        ...
        wrapped_paragraphs = [textwrap.fill(p, width=chars_per_line) for p in paragraphs]
        test_text = "\n\n".join(wrapped_paragraphs)

        bbox = draw_temp.textbbox((0, 0), test_text, font=font, spacing=6)
    ...
    paragraphs = text.split("\n\n")
    wrapped_paragraphs = [textwrap.fill(p, width=max(10, int(max_w / (font.getlength("a") * 1.2)))) for p in paragraphs]
    return font, "\n\n".join(wrapped_paragraphs)
...
def main():
    ...
    if isinstance(summary_keys, list):
        paragraphs = []
        for key_path in summary_keys:
            extracted_text = extract_json_node(base_node, key_path)
            if extracted_text:
                paragraphs.append(str(extracted_text))
        summary = "\n\n".join(paragraphs) if paragraphs else "No content available."
    else:
        summary = extract_json_node(base_node, summary_keys) or "No summary available."
